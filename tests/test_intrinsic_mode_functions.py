...
class TestIntrinsicModeFunction(unittest.TestCase):
    ...
    def test_properties(self):
        ...
        self.assertAlmostEqual(sum(imf.default_probability_dict.values()), 1.0, places=5)
    ...
    def test_add_noise_uses_rms_and_rng(self):
        ...
        self.assertAlmostEqual(float(np.std(n1)), expected_std, delta=0.05 * expected_std)
    ...
    def test_get_base_and_choice_imfs(self):
        imf = self._legacy_like(min_base_imfs=1, max_base_imfs=2, min_choice_imfs=1, max_choice_imfs=2)
    ...
    def test_get_choice_imfs_am_signal(self):
        am_prob_dict = {k: (1.0 if k == "generate_am_signal" else 1e-6) for k in ALL_IMF_DICT}
    ...
    def test_max_choice_imfs_is_inclusive(self):
        ...
        out = imf.get_choice_imfs(np.zeros(64), rng=np.random.RandomState(0), seq_length=64)
    ...
    def test_wavelet_bursts_finite_and_localized(self):
        ...
        out = imf.get_wavelet_imfs(np.zeros(256), rng=np.random.RandomState(1), seq_length=256)
        self.assertTrue(np.all(np.isfinite(out)))
        self.assertFalse(np.allclose(out, 0.0))
    ...
    def test_trend_changes_low_frequency_structure(self):
        ...
        y0 = base.generate(np.random.RandomState(0), seq_length=256, num_channels=1)[:, 0]
        y1 = with_trend.generate(np.random.RandomState(0), seq_length=256, num_channels=1)[
            :, 0
        ]
    ...
    def test_amplitude_decay_with_freq(self):
        imf = IntrinsicModeFunction(amplitude_decay_with_freq=True, amplitude_decay_gamma=0.5)
