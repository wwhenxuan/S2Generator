...
class KernelSynth(BaseExcitation):
    ...
    def generate(
        ...
        :param seq_length: The number of points in the input distribution.
        :param num_channels: The dimensionality of the input distribution.
        :return: The time series generated by KernelSynth.
        """
        # If the length of the input changes, the kernel bank needs to be adjusted.
        if self.length != seq_length:
            # Record the first length information or update the latest length
            self.length = seq_length
            # Create the latest library
            self._kernel_bank = self._update_kernel_bank(length=seq_length)

        return np.vstack(
            [
                self.generate_kernel_synth(rng=rng, length=seq_length)
