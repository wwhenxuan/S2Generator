...
Chronos uses the inverse of this process: randomly compose GP kernels to generate new time series. The data generation process is shown below:

.. image:: ../../_static/algorithm_kernel_synth.jpg
   :width: 100%
   :align: center
...
.. list-table::
   ...
   * - Kernel
     ...
     - Hyperparameters
   * - Constant
     - :math:`\mathcal{K}_{\mathrm{Const}}(x, x') = C`
     - :math:`C=1`
   * - White Noise
     - :math:`\mathcal{K}_{\mathrm{White}}(x, x') = \sigma_n \cdot \mathrm{1}_{(x=x')}`
     - :math:`\sigma_n \in \{ 0.1, 1 \}`
   * - Linear
     - :math:`\mathcal{K}_{\mathrm{Lin}}(x, x') = \sigma ^ 2 + x \cdot x'`
     - :math:`\sigma \in \{ 0, 1, 10 \}`
   * - RBF
     - :math:`\mathcal{K}_{\mathrm{RBF}}(x, x') = \mathrm{exp} \left ( - \frac{\left \| x - x' \right \| ^ 2}{2 l ^ 2} \right )`
     - :math:`l \in {0.1, 1, 10}`
   * - Rational Quadratic
     - :math:`\mathcal{K}_{\mathrm{RQ}}(x, x') = \left ( 1 + \frac{\left \| x - x' \right \| ^ 2}{2 \alpha} \right )`
     - :math:`\alpha \in \{ 0.1, 1, 10 \}`
   * - Reriodic
     - :math:`\mathcal{K}_{\mathrm{Per}}(x, x') = \mathrm{exp} \left ( - 2 \sin ^ 2 \left ( \pi \frac{\left \| x - x' \right \|}{p} \right ) \right )`
     - :math:`p \in \left \{ 24, 48, 96, 168, 336, 672, 7, 14, 30, 60, 365, 730, 4, 26, 52, 6, 12, 40, 10 \right \}`
...
for i in range(3):
    for j in range(2):
        ...
        time_series = kernel_synth.generate(rng=rng, num_channels=1, seq_length=512)
