...
This method came from SNIP in the domain of Symbolic Regression. It has subsequently further extended in SymTime to make it applicable to time series data.
...
Finally we randomly generate the weights of each distribution to mix them with weights. Specifically for the :math:`k` distributions we generate :math:`\{ w_1, w_2, \cdot, w_k \} \sim \mathcal{U}(0, 1)` and normalize it to :math:`\sum_{j}^{k}w_j = 1`.
