...
**[Sep. 2026]** We add an [IQSimulator](https://github.com/wwhenxuan/S2Generator/blob/main/s2generator/simulator/iq_simulator.py) for generating complex baseband / analytic IQ channel traces: the phase is taken from a real time series (or ARMA excitation), while the two-sided Doppler spectrum is matched to packaged [DeepMIMO](https://github.com/DeepMIMO/DeepMIMO) CSI. See the [IQ example](https://github.com/wwhenxuan/S2Generator/blob/main/examples/simulator/iq_simulator.py) and the [DeepMIMO data example](https://github.com/wwhenxuan/S2Generator/blob/main/examples/tools/deepmimo_iq_data.py).

**[Aug. 2026]** Recently, multivariate time series generation algorithms based on structural causal models (SCM) have been widely used: [TabPFNv3](https://arxiv.org/abs/2605.13986), [CauKer](https://arxiv.org/abs/2508.02879), and [TiRex-2](https://arxiv.org/abs/2607.01204). Therefore, we specifically reproduced the corresponding generation algorithms in the [scm module](https://github.com/wwhenxuan/S2Generator/tree/main/s2generator/scm). See the [examples](https://github.com/wwhenxuan/S2Generator/tree/main/examples/scm).
...
For a **user-specified** symbolic expression (complex system), use [`CustomSymbolGenerator`](https://github.com/wwhenxuan/S2Generator/blob/main/s2generator/symbol/generators.py): bind `f(.)` once, then sample excitation `X` and obtain `Y=f(X)`.
